...
```{r}
descdist(lorazepam$proportion)

fit.beta <- fitdist(lorazepam$proportion, "gamma")
fit.beta <- fitdist(lorazepam$proportion, "beta")
plot(fit.gamma)
plot(fit.beta)

mGA <- histDist(lorazepam$proportion, "GA", density = T, main = "Gamma")
mBE <- histDist(lorazepam$proportion, "BE", density = T, main = "Beta")
GAIC(mGA, mBE)

lorazepam_mod <- glm(proportion ~ treatment + time_min + treatment:time_min, family = Gamma(link = "inverse"), data = lorazepam) 

backward.lorazepam <- stepGAIC(lorazepam_mod, direction = "backward", trace = F)

formula(backward.lorazepam) # get the final formula , proportion ~ treatment + time_min
backward.lorazepam$anova 
summary(backward.lorazepam) 
```
